...
export function NewsCard({ news, onDelete }: NewsCardProps) {
  const imageUrl = news.image
    ? `${process.env.NEXT_PUBLIC_API_STORAGE}/${news.image}`
    : extractImageUrl(news.content);

  const contentText = truncateText(stripHtmlTags(news.content), 150);
      ...
      <div className="w-full md:w-4/5">
        <CardHeader>
          <CardTitle className="text-sm font-medium hover:underline md:text-base">
            <Link href={`/dashboard/news/${news.id}`}>{news.title}</Link>
          </CardTitle>
